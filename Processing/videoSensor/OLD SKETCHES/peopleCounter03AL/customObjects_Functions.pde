  ...
  for (int n = 0; n < bd.getBlobNb(); n++) {//cycling through the total number of detected blobs
    ...
    if (b != null && realBlob(b)) { //if it exists and if it's validated as a 'real blob'
      ...
      for (int i = objectList.size()-1; i >= 0; i--) { //loop through the object list backwards
        ...
        if (dist(b.xMin*width, b.yMin*height, blob.xpos[blob.xpos.length - 1], blob.ypos[blob.ypos.length - 1]) < thresh) {
          blob.updatePosition(b.xMin*width, b.yMin*height, millis(), year() + ":" + month() + ":" + day() + ":" + hour() + ":" + minute() + ":" + second());
          isItOld = true;
        }
      }
      if (!isItOld && realBlob(b)) { // if it's not an old blob and it's a verified blob.
        objectList.add(new TrackedBlob(b.xMin*width, b.yMin*height, millis(), year() + ":" + month() + ":" + day() + ":" + hour() + ":" + minute() + ":" + second()));
        println("new object detected, number: " + objectList.size());
      }
    }
  }
}
...
void evaluateOldBlobs() { //bd is the local instance of the blob detection
  for (int i = objectList.size()-1; i >= 0; i--) { //loop through the object list backwards
    ...
    if (blob.mSecond[blob.mSecond.length-1] < millis() - 1000) {
      if (blob.xpos[0] < blob.xpos[blob.xpos.length-1]) {
        leftToRight++;
        objectList.remove(i);
      }
      if (blob.xpos[0] > blob.xpos[blob.xpos.length-1]) {
...
boolean realBlob(Blob realB) {
  ...
  return (realB.w*width > blobMinSize) && (realB.w*height > blobMinSize) && (realB.yMin*height < height-50);
}
