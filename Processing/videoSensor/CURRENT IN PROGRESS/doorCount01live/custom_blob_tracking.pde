  ...
  for (int n = 0; n < bd.getBlobNb(); n++) {//cycling through the total number of detected blobs
    ...
    if (b != null && realBlob(b)) { //if it exists and if it's validated as a 'real blob'
      ...
      for (int i = objectList.size()-1; i >= 0; i--) { //loop through the object list backwards
        ...
        if (dist(b.xMin*width + (b.w*width/2), b.yMin*height + (b.h*height/2), blob.xpos[blob.xpos.length - 1], blob.ypos[blob.ypos.length - 1]) < thresh) {
          blob.updatePosition(b.xMin*width + (b.w*width/2), b.yMin*height + (b.h*height/2), millis(), year() + ":" + month() + ":" + day() + ":" + hour() + ":" + minute() + ":" + second());
          isItOld = true;
        }
      }
      if (!isItOld && realBlob(b)) { // if it's not an old blob and it's a verified blob.
        objectList.add(new TrackedBlob(b.xMin*width + (b.w*width/2), b.yMin*height + (b.h*height/2), millis(), year() + ":" + month() + ":" + day() + ":" + hour() + ":" + minute() + ":" + second()));
      }
    }
  }
}
...
// - - person-blob verification based on blob size (and now position)
boolean realBlob(Blob realB) {
  // realBlob is true if the width and the height of the blob are both above the minimum blob size
  return (realB.w*width > blobMinWidth) && (realB.w*height > blobMinHeight) && (realB.xMin*width + (realB.w*width/2) > leftLimit) && (realB.yMin*height + (realB.h*height/2) > topLimit);
}

// - - verification whether a given coordinate set is within the "target" (Based on minimum and maximum.. so it's a square target.
